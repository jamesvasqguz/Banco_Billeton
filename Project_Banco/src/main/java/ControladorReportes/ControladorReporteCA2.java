 ...
 * @author jara
 */
@WebServlet(name = "ControladorReporteCA2", urlPatterns = {"/ControladorReporteCA2"})
public class ControladorReporteCA2 extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String fechaInicio = request.getParameter("f1");
            String fechaFinal = request.getParameter("f2");
            System.out.println("Inicio " + fechaInicio);
            System.out.println("Final " + fechaFinal);
            CAReporte2 reportar = new CAReporte2();

            List<CAReporte2> exportado = reportar.reporte2Cajero(codigoUsuario, hora1, hora2, fechaInicio, fechaFinal);

            double saldoCaja = reportar.saldoCaja(codigoUsuario, hora1, hora2, fechaInicio, fechaFinal);

            response.setContentType("application/pdf");
            response.setHeader(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=ReporteCajero2" + nombreUsuario + " Del " + fechaInicio + " al " + fechaFinal + " .pdf");

            //List<Cliente> Histo = gestor.reporte5(t, s);
            File file = new File(request.getServletContext().getRealPath("/Reports/CAReport2.jrxml"));
